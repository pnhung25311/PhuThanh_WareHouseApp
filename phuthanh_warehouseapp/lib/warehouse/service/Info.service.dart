...
class InfoService {
  ...
  Future<Map<String, dynamic>> LoadGuarantee() async {
    try {
      const apiClient = ApiClient();
      final response = await apiClient.get("dynamic/get-all/vwGuarantee");
      ...
      print("==================> Raw JSON length: ${rawList.length}");
      // print(rawList); // chỉ print khi debug, tránh log quá dài

      final List<Guarantee> guarantees = [];
      for (final item in rawList) {
        try {
          guarantees.add(Guarantee.fromJson(item as Map<String, dynamic>));
        } catch (parseError, stack) {
          print("Lỗi parse item: $parseError");
          print("Item lỗi: $item");
          print(stack);
          // Tiếp tục với item tiếp theo thay vì crash toàn bộ
        }
      }
      ...
      print("Lỗi tổng khi load guarantee: $e");
      print(stack);
      return {"isSuccess": false, "statusCode": 0, "body": e.toString()};
    }
  }
